...
// go中包名不可以有中文
// 每个项目一般主文件命名为main.go 定义包名为package main 里面包含一个main函数（主函数）
// 真正项目中 文件名（xxx.go）及包名（package xxx）要和所在的文件夹（xxx）命名要一致 因这个项目是学习项目 为方面章节编码 所以命名不一致
// 在go文件中输入快捷命令 pkgm 可快速生成一个基础代码体
// 如果变量名 常量名 函数名首字母大写 则可以被其他包访问 如果首字母小写 只能在本包中使用
// 可以理解为：首字母大写是公开的 首字母小写是私有的（详见第六章节）
